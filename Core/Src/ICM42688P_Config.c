...
void ICM42688P_LoadDefaultConfig(ICM42688P_Config* config)
{
    ...
    config->bank1.GYRO_CONFIG_STATIC6 = 0x00;  // 工厂校准值
    config->bank1.GYRO_CONFIG_STATIC7 = 0x00;
    config->bank1.GYRO_CONFIG_STATIC8 = 0x00;
    config->bank1.GYRO_CONFIG_STATIC9 = 0x00;
                          ...
                          uint8_t ui_filt_ord, uint8_t ui_filt_bw)
{
    ...
    config->bank0.GYRO_CONFIG0 = 
        ((fs_sel & 0x07) << 5) | (odr & 0x0F);
    
    // GYRO_CONFIG1: UI_FILT_ORD[3:2]
    config->bank0.GYRO_CONFIG1 = 
        (config->bank0.GYRO_CONFIG1 & 0xF3) | ((ui_filt_ord & 0x03) << 2);
...
{
    config->bank0.INT_CONFIG = 
        ((int2_mode & 0x01) << 5) |
        ((int2_drive & 0x01) << 4) |
        ((int2_polarity & 0x01) << 3) |
        ((int1_mode & 0x01) << 2) |
        ((int1_drive & 0x01) << 1) |
        (int1_polarity & 0x01);
}
                             ...
                             uint8_t delt, uint16_t deltsqr, uint8_t bitshift)
{
    ...
    config->bank1.GYRO_CONFIG_STATIC5 = 
        ((bitshift & 0x0F) << 4) | ((deltsqr >> 8) & 0x0F);
}
                              ...
                              uint8_t delt, uint16_t deltsqr, uint8_t bitshift)
{
    ...
    config->bank2.ACCEL_CONFIG_STATIC2 = 
        ((delt & 0x3F) << 1) | (enable & 0x01);
                           ...
                           uint8_t clk_sel, uint8_t rtc_mode, uint8_t pin9_func)
{
    ...
    config->bank0.INTF_CONFIG1 = 
        (config->bank0.INTF_CONFIG1 & 0xF8) |
        ((rtc_mode & 0x01) << 2) |
        (clk_sel & 0x03);
    
    // INTF_CONFIG5: PIN9_FUNCTION[2:1]
    config->bank1.INTF_CONFIG5 = 
        (config->bank1.INTF_CONFIG5 & 0xF9) | ((pin9_func & 0x03) << 1);
}
...
void ICM42688P_ConfigTemperature(ICM42688P_Config* config, uint8_t filt_bw)
{
    ...
    config->bank0.GYRO_CONFIG1 = 
        (config->bank0.GYRO_CONFIG1 & 0x1F) | ((filt_bw & 0x07) << 5);
}
...
uint8_t ICM42688P_ReadConfig(ICM42688P_Config* config)
{
    ...
    ICM42688P_ReadRegister(ICM42688P_REG_BANK0_GYRO_ACCEL_CONFIG0, &config->bank0.GYRO_ACCEL_CONFIG0, 1);
    ...
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC4, &config->bank1.GYRO_CONFIG_STATIC4, 1);
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC5, &config->bank1.GYRO_CONFIG_STATIC5, 1);
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC6, &config->bank1.GYRO_CONFIG_STATIC6, 1);
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC7, &config->bank1.GYRO_CONFIG_STATIC7, 1);
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC8, &config->bank1.GYRO_CONFIG_STATIC8, 1);
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC9, &config->bank1.GYRO_CONFIG_STATIC9, 1);
    ICM42688P_ReadRegister(ICM42688P_REG_BANK1_GYRO_CONFIG_STATIC10, &config->bank1.GYRO_CONFIG_STATIC10, 1);
